...
- **`collect.py`** — produces `jobs.csv` (columns: `company`, `ir_summary`, `press_release`). Two modes:
  - Default: writes the hardcoded `SAMPLE_DATA` list (5 synthetic companies, one per gap pattern — the earlier version padded this to 20 by duplicating the 5 patterns via `_EXTRA_NAMES`; that duplication was removed since it added no real variety). The texts are written at realistic 有報 length (IR ~400–600 chars, PR ~250–400 chars) with **partial vocabulary overlap** between each company's IR and PR — an earlier, much shorter version had zero token overlap, which pinned `gap_score` at 100 and `kw_match` at 0 for every company and made the ranking degenerate to sentiment-only. Designed gap ordering: A・C・E (large) ≫ B・D (small). D社's press release deliberately ends with a disclaimer sentence to exercise `_strip_boilerplate`.
...
- **`engine.py`** — all scoring logic; both `app.py` and the CLI import from here. Key entry points: `analyze_company(company, ir_summary, press_release) -> dict` and `analyze_all(csv_path) -> pd.DataFrame` (sorted ascending by `realness`, i.e. worst gap first).
  ...
  - `gap_score` (0–100, higher = more divergence): `1 − similarity` of `ir_summary` vs `press_release` (boilerplate-stripped). In MeCab/fugashi mode the similarity is a **0.6/0.4 blend** (`_GAP_WORD_WEIGHT`) of (a) word-level TF-IDF cosine using `_tokenize_split` — `tokenize(text, split_compounds=True)`, which emits each compound noun *plus* its constituent noun morphemes so 海外向け販売/海外売上 partially match on 海外 — and (b) `_char_ngram_similarity`, a char 2–3gram cosine over kanji/katakana/Latin only (hiragana is stripped via `_CONTENT_CHAR_RE` so particles don't inflate similarity). Pure word-exact-match cosine sat at ~0 on short docs, which is why the blend exists. In chargram mode it's a single char_wb cosine as before. Note `split_compounds` is used **only** here; keyword extraction still uses plain `tokenize()` (compounds only).
